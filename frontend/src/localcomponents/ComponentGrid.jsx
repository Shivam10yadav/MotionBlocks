      ...
      <div className="grid gap-4 sm:grid-cols-2 sm:gap-6 xl:grid-cols-3">
        {filtered.map((component) => {
          ...
          return (
            ...
            >
              <div className="blueprint-grid relative h-44 overflow-hidden border-b border-[#23262F] bg-[#0B0D12]">
                {[
                  ...
                  <span
                    key={i}
                    className={`pointer-events-none absolute z-10 h-3 w-3 border-[--teal]/40 ${pos}`}
                  />
                ))}

                {Preview ? (
                  <div className="pointer-events-none absolute left-1/2 top-1/2 w-[640px] origin-center -translate-x-1/2 -translate-y-1/2 scale-[0.28]">
                    <Preview />
                  </div>
                ) : (
