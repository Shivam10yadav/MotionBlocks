...
const LogoIcon = () => (
  <div className="relative flex h-7 w-7 items-center justify-center rounded-lg bg-gradient-to-br from-[#FF7A45] to-[#5EEAD4] shadow-[0_0_12px_rgba(255,122,69,0.35)] transition-transform duration-300 group-hover:scale-105">
...
export function Navbar({ githubUrl = "https://github.com/Shivam10yadav/MotionBlocks", onBrowseClick }) {
  ...
  return (
    <header className="pointer-events-none fixed left-0 right-0 top-0 z-50 flex justify-center px-4 pt-3 sm:pt-5 [--ember:#FF7A45] [--teal:#5EEAD4]">
      {/* Dark Floating Glass Container */}
      <nav
        className={`pointer-events-auto w-full max-w-[1100px] rounded-full border border-white/10 bg-[#08090E]/60 backdrop-blur-2xl shadow-[0_8px_32px_0_rgba(0,0,0,0.5)] transition-all duration-300 hover:border-[#FF7A45]/30 hover:shadow-[0_0_30px_rgba(255,122,69,0.12)] ${
      ...
      >
        <div className="flex items-center justify-between">
          ...
          <Link to="/" className="group flex items-center gap-2.5 focus:outline-none">
            <LogoIcon />
            <span className="font-display text-base font-bold tracking-tight text-white transition-colors duration-200 group-hover:text-[--ember]">
              Motion<span className="text-[--teal]">Blocks</span>
            </span>
          </Link>

          {/* Nav Links Capsule */}
          <div className="hidden items-center gap-1 rounded-full border border-white/[0.08] bg-white/[0.03] px-2 py-1 md:flex">
            {navLinks.map((link) => {
              ...
              return (
                <Link
                  key={link.name}
                  to={link.href}
                  className={`group relative flex items-center gap-1.5 rounded-full px-3.5 py-1 text-xs font-semibold tracking-wide transition-all duration-200 ${
                ...
                >
                  ...
                  {/* Subtle active / hover pill indicator */}
                  <span
          ...
          <div className="hidden items-center gap-2.5 md:flex">
            ...
            <a
              ...
              className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.04] px-3.5 py-1.5 font-code text-[11px] font-medium text-white transition-all duration-200 hover:border-white/25 hover:bg-white/[0.08] hover:shadow-[0_0_15px_rgba(255,255,255,0.05)] active:scale-[0.97]"
            >
