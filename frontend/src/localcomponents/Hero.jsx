...
/* -----------------------------------------------------------------------------
 * VECTOR BRAND LOGO COMPONENTS (Dark Mode Fills)
 * -------------------------------------------------------------------------- */
...
const BRAND_LOGOS = [
  ...
  // Framer Motion
  () => (
    <div className="flex items-center justify-start font-bold text-sm md:text-base gap-2 text-zinc-300 opacity-60 hover:opacity-100 transition-opacity duration-300">
      <svg
        viewBox="0 0 14 21"
        role="presentation"
  ...
  () => (
    <svg
      className="h-[14px] sm:h-[18px] fill-zinc-300 select-none opacity-60 hover:opacity-100 transition-opacity duration-300"
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 394 79"
    >
...
export function Hero({
  word1 = "Motion",
  word2 = "Blocks.",
  description = "Ready-to-use, beautifully animated React components tailored for modern web apps. Copy, paste, and elevate your interface in seconds.",
  primaryCta = "Browse Components",
  primaryCtaMobile = "Browse",
  ...
  const themeColors = [
    "rgba(82, 82, 91, 0.4)",  // Zinc 600
    "rgba(113, 113, 122, 0.5)", // Zinc 500
    "rgba(161, 161, 170, 0.6)", // Zinc 400
    "rgba(6, 182, 212, 0.7)",   // Cyan 500 accent
  ];
  ...
  return (
    <div className="relative w-full min-h-[100dvh] bg-zinc-950 text-zinc-100 flex flex-col justify-between md:justify-center md:gap-6 py-8 md:py-0 px-2 sm:px-6 overflow-hidden select-none isolate">
      <style>{`
        @keyframes marquee {
        ...
        .tahoe-glass-text-dark {
            color: transparent;
            background: linear-gradient(135deg, #ffffff 0%, rgba(255, 255, 255, 0.5) 30%, rgba(6, 182, 212, 0.8) 50%, rgba(255, 255, 255, 0.3) 70%, #ffffff 100%);
            background-size: 200% auto;
            -webkit-background-clip: text;
            background-clip: text;
            -webkit-text-stroke: 1px rgba(255, 255, 255, 0.25);
            filter: drop-shadow(0 15px 35px rgba(0, 0, 0, 0.9)) drop-shadow(0 0 15px rgba(6, 182, 212, 0.2));
      ...
      <div className="absolute inset-0 z-0 pointer-events-none">
        ...
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,transparent_0%,rgba(9,9,11,0.95)_100%)] pointer-events-none" />
      </div>
      ...
      <div className="flex flex-col items-center justify-center text-center order-1 md:order-1 mt-28 sm:mt-0 pointer-events-none w-full">
        <h1 className="tahoe-glass-text-dark flex flex-row items-center justify-center gap-1.5 sm:gap-4 lg:gap-6 px-1 w-full flex-wrap text-[2.8rem] xs:text-[3.2rem] sm:text-6xl md:text-8xl lg:text-9xl leading-none">
      ...
      <div className="flex flex-col items-center justify-center text-center my-auto md:my-0 order-2 md:order-2 px-1 w-full pointer-events-none">
        <p className="text-sm sm:text-lg md:text-xl font-light text-zinc-400 max-w-[95%] sm:max-w-md md:max-w-xl px-1 leading-relaxed">
          {description}
        </p>

        <div className="block md:hidden w-full mt-14 pointer-events-auto">
          <div className="text-[11px] uppercase tracking-wider text-zinc-500 font-medium mb-5">
      ...
      >
        ...
        <a
          ...
          className="relative inline-flex h-10 md:h-12 items-center justify-center gap-1.5 md:gap-2 rounded-xl bg-zinc-900/80 hover:bg-zinc-800/80 px-4 md:px-8 text-xs md:text-sm font-semibold text-zinc-200 border border-zinc-800 shadow-[inset_0_1px_1px_rgba(255,255,255,0.08)] backdrop-blur-md transition-transform duration-200 hover:scale-[1.02] active:scale-[0.98] cursor-pointer"
        >
          <FaGithub className="w-3.5 h-3.5 md:w-4 md:h-4 text-zinc-300" />
      ...
      >
        <span className="text-xs uppercase tracking-wider text-zinc-500 font-medium select-none">
          Built for modern tech stacks
        </span>
        <div className="relative w-full max-w-5xl overflow-hidden [mask-image:linear-gradient(to_right,transparent,black_15%,black_85%,transparent)]">
