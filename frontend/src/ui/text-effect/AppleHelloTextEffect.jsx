...
export const AppleHelloTextEffect = ({ className = "", speed = 1, onAnimationComplete, ...props }) => {
  const calc = (x) => x * speed;

  return (
    <motion.svg
      className={`h-20 text-[#F4F3F1] ${className}`}
...
// Demo wrapper: replays the animation on click so it's interactive in the preview
export const AppleHello = () => {
  const [key, setKey] = useState(0);

  return (
    <div className="flex flex-col items-center gap-6">
      <AnimatePresence mode="wait">
        <AppleHelloTextEffect key={key} />
      </AnimatePresence>

      <button
        onClick={() => setKey((k) => k + 1)}
        className="rounded-full border border-[#23262F] bg-[#111319] px-4 py-2 font-code text-xs uppercase tracking-widest text-[#8B8D98] transition hover:border-[#5EEAD4]/40 hover:text-[#5EEAD4]"
      >
        Replay
      </button>
    </div>
